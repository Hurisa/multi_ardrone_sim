...
void setNeighbourhood(vector<Ground*>& ground, vector<double>& CosThetaNeighbours, vector<double>&  SinThetaNeighbours, vector<double>& xPosSeparation, vector<double>&  yPosSeparation, int j){
...
void flocking(vector<Ground*>& ground, int j, int alpha){
	...
	setNeighbourhood(ground,CosThetaNeighbours, SinThetaNeighbours, xPosSeparation, yPosSeparation, j);
	...
	if (SeVel.angular.z==0){
		...
		avoiding.data = false;
		p.angular.z=alpha*AlVel.angular.z;
	}
	else{
		//cout<<paramStr<<endl;

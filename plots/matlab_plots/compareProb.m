...
TPerCent=input('Time to be analysed (0 to 1): ');
global realVar
...
if size(Afiles,1)==size(NAfiles,1)
    ...
    for f=1:size(Afiles,1)        
        ProbAreaCell{1,f}=ProbD(strcat(Alignd,'/',Afiles(f).name),cellSize,TPerCent);
        ProbAreaCell{2,f}=ProbD(strcat(nAlignd,'/',NAfiles(f).name),cellSize,TPerCent);
    end
else
    disp('Number of parameter files do not match');
end

TotalCells=input('How many cells are there? ');
%% Plot 1-coverage
%plotProb(1,ProbAreaCell,Afiles,NAfiles,TotalCells)


%% Plot 2-coverage
plotProb(2,ProbAreaCell,Afiles,NAfiles,TotalCells)


%% Plot 3-coverage
%plotK(3,ProbAreaCell,Afiles,NAfiles,TotalCells)


%%

function plotProb(k,KCovCell,Afiles,NAfiles,limitX)
global realVar
figure
hold on
box on
xlower=input('Plot beginnig from x= ');
x=[xlower:limitX];
